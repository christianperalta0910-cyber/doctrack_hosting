    ...
    @else
        <form method="POST" action="{{ route('originator.documents.routeCustom', $document) }}" class="space-y-4">
            ...
            {{-- Grouped by department, head(s) listed first within each
                 (see DocumentController::selectApprovers()) — a document
                 that "just needs the head's sign-off" is exactly the
                 case this grouping exists to make obvious at a glance,
                 instead of hunting through one flat alphabetical list. --}}
