{{--
    SLA Override Queue results — split out from sla_queue.blade.php so the
    same markup can be rendered two ways: a normal full page load, and a
    fragment returned by AdminController::slaQueueRefresh() for the
            ...
            @foreach($container->documents as $documentId => $stageAssignments)
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-2">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-rejected-50 text-rejected-700 ring-1 ring-inset ring-rejected-500/20">SLA Violated</span>
                        @if(!$container->is_batch)
                    ...
                    {{-- Full stage pipeline for this document's category, so admins can see
                         ...
                         violated stage currently needs a decision. Only the earliest
                         (lowest sequence_order) violated stage is highlighted and actionable
                         below; once it's resolved, the next violated stage (if any) surfaces
                         the same way on reload. --}}
                    <div class="mb-4">
                        <x-workflow-stage-list :document="$doc" :highlight-assignment-id="$activeAssignment->assignment_id" />
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4 rounded-lg border border-rejected-500/30 bg-rejected-50/40 p-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ring-1 ring-inset bg-rejected-50 text-rejected-700 ring-rejected-500/20">SLA Violated</span>
                        ...
                        <form method="POST" action="{{ route('admin.sla.override', $activeAssignment) }}" class="flex flex-col sm:w-64 gap-2">
                            @csrf
                            <textarea name="comments" rows="1" placeholder="Override reason (required if rejecting)…"
...
@empty
    <div class="bg-white rounded-xl shadow-card border border-surface-200 p-12 text-center">
        <p class="text-sm text-surface-500">No SLA violations pending override.</p>
    </div>
@endforelse
                            ...
                            @foreach($container->assignments as $reviewAssignment)
                                <li class="px-4 py-2.5 bg-surface-50/50">
                                    ...
                                    <p class="text-xs text-surface-500">
                                        ...
                                        SLA violated <span data-live-time="{{ optional($reviewAssignment->sla_expires_at)->timestamp }}">{{ optional($reviewAssignment->sla_expires_at)->diffForHumans() }}</span> &middot;
