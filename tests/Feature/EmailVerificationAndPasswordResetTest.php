...
it('resets the password with a valid token and allows logging in with the new password', function () {
    ...
    // Logging in with the new password is what proves it was actually
    // accepted.
    $this->post(route('login.attempt'), ['email' => 'canreset@example.test', 'password' => 'A-brand-new-password1'])
        ->assertRedirect(route('originator.dashboard'));
});
