...
it('does not block a login attempt for a different email after another one is throttled', function () {
    ...
    // A different email's correct login isn't blocked by the OTHER
    // email's throttle — reaching the dashboard, rather than "Too many
    // login attempts", is what proves the two counters are independent.
...
it('clears the throttle counter on a successful login', function () {
    ...
    // The counter only clears once login() itself succeeds (see
